...
本 Spec 是本轮唯一允许产出最终 APK 和操作物理手机的阶段。B0-1 至 B3-2 只交付自动化与
Pixel_7 模拟器证据；进入本 Spec 后才依次执行最终构建、手机外部备份、覆盖安装和真机矩阵。
...
1. 在安装任何 MVP-B 构建前记录当前版本和匿名聚合，验证 schema v1 latest 并导出外部备份；
2. 构建最终 APK 并从当前真实 schema v1 一次覆盖安装；启动时必须按 v1 → v2 → v3 → v4 → v5
   的已测迁移链执行，不卸载、不清数据；
3. 对比聚合、历史、当前模型和规则；
4. 连续自然记录观测；
5. 验证抽样负担；
...
若技术真机矩阵通过但自然证据尚不足，工程可标为 complete，两个产品状态保持 inconclusive，
生产学习 / 自动应用门保持关闭并进入后续自然观察；不得为了本次安装强行开启参数变化。

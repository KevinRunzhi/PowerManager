...
- V2359A 已从真实 schema v1 无清数据覆盖升级到 schema v5，原七表匿名聚合保持一致，迁移后
  `integrity_check=ok` 且只有一个 active 初始个人模型。
...
4. 实体手机后台/重启、跨生活日、备份恢复和真实通知路径；首次覆盖升级已通过。

当前不得使用测试 fixture、ADB 注入或模拟器合成数据把状态改写为 `validated`。

## 4. 后续更新规则

首次覆盖安装前已经记录实体手机版本、外部备份和匿名聚合；完成剩余 MVP-B 总验收矩阵后继续更新
本报告：

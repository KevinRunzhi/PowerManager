...
只有同时满足下列条件才允许在模拟器构建并运行 schema v2 版本：

- B0-1 自动化与模拟器 latest 备份可解析且恢复演练通过；
- 当前数据体检无 P0 一致性错误；
...
3. 通过 `flutter run` 覆盖安装 v2 模拟器构建，不卸载、不清数据；
...
MVP-B 全部工程阶段和模拟器总矩阵通过后，在物理手机先执行 B0-1 外部备份，再使用最终版本
一次完成 schema v1 → v5 覆盖升级。该最终路径必须逐步调用并验证 v1 → v2 迁移；不得因跳过
本阶段真机安装而提供 v1 → v5 的重建或清库捷径。
